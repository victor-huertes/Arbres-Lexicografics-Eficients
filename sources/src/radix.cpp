...
// Inicialitzar el trie amb un text
void RadixTrie::init(const string &text, int mode)
{
...
bool RadixTrie::empty() const
{
    ...
    if (!root)
        return true; // Cas extrem si l'arrel és nullptr

    // Si l'arrel no té fills, el trie està buit.
    return root->children.empty();
}

size_t RadixTrie::calculate_node_memory(RadixNode *node) const
{
    if (!node)
        return 0;
    ...
    // Memoria del nodo mismo (struct RadixNode)
    memory += sizeof(RadixNode);

    // Memoria del string label (capacidad del string)
    memory += node->label.capacity();

    // Memoria del vector de posiciones
    memory += sizeof(int) * node->positions.capacity();

    // Memoria del unordered_map children
    // Aproximación: cada entrada tiene overhead del map + key + unique_ptr
    memory += node->children.size() * (sizeof(char) + sizeof(unique_ptr<RadixNode>) + 32); // overhead estimado del map

    // Recursivamente calcular memoria de los hijos
    for (const auto &pair : node->children)
    {
        memory += calculate_node_memory(pair.second.get());
    }
